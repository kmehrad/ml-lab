...
| exp-021 | 2026-07-08 | Greedy hillclimb (`src/hillclimb.py`), 15 candidates | **8.73818** | **8.70364** | — | Forward-selection ensemble over every model variant from exp-001..020 (equal-weight avg, repetition allowed). Picked 7 models (`lgbm_lgbm_lr02, xgb, lgbm_lgbm_d8, xgb_xgb_ratios, xgb_xgb_lr05, lgbm_lgbm_d7, cat`), each 1/7 weight. **−0.00353 vs the submitted blend, confirmed to win in all 5 individual folds** (not just on average) — a real, if small, signal. **SUBMITTED: public LB 8.70364 / private 8.73083.** Public LB is marginally *worse* (+0.00089) and private marginally *better* (−0.00026) than the previous best — a wash within LB sampling noise, consistent with the OOF gain being real but small relative to what the smaller LB test split can resolve. |
| — | 2026-07-08 | Hillclimb, lean 6-candidate set (best-per-family only) | 8.74105 | — | — | Sanity check with a smaller, non-cherry-picked candidate set: converges to a plain 2-model blend of `lgbm_lgbm_lr02` + `xgb` after 1 step, confirming cat/nn add nothing here. Only −0.00066 vs the submitted blend — noise-level; **not what was submitted** (the full 15-candidate result above was). |
