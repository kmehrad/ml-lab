...
#
# CatBoost/XGBoost GPU training and the pytabkit NN diversity step run on the box; the Mac has
# no CUDA. Code is pushed with rsync (the box is not a git checkout); the git-ignored data/ and
# generated experiments/artifacts/ are synced too so training and blending have their inputs
# and we can pull results back.
#
# Usage:
...
#
# Example:
#   scripts/remote_run.sh all uv run python -m src.train --model cat --device cuda \
#       --trees 6000 --lr 0.02 --tag cat_tune1
set -euo pipefail

BOX="${HD_BOX:-kamron@192.168.0.92}"
REMOTE_DIR="${HD_REMOTE_DIR:-HeartDisease}"     # relative to the box user's ~
LOCAL_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
# The box authenticates with the passphrase-less `dragon` key; use it directly so this works in a
# fresh shell (no ssh-agent needed). Override with HD_SSH_KEY.
SSH_KEY="${HD_SSH_KEY:-$HOME/.ssh/dragon}"
